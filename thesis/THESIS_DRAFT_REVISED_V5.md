...
第三章 核心算法设计与实验分析：给出数据预处理、Autoencoder 缺失值填充、Baseline 与 ComBat 批次效应校正、下游分析等四类核心算法的完整设计与工程实现，并在多个数据集上进行定量对比实验，展示差异代谢物分析与通路富集分析的结果，验证系统通用性。

第四章 系统设计与工程实现：阐述系统需求分析、前后端分离总体架构、数据库设计、模块划分与数据流设计，并介绍技术栈选型、后端 API、前端组件与可视化实现，展示系统各功能模块界面及测试结果。

第五章 结论与展望：总结本文的主要工作与贡献，指出系统的局限性，并展望后续改进方向。
...
本章详细阐述了系统四类核心算法的设计、实现与实验验证。数据预处理算法以缺失率过滤、log1p 变换和 Z-score 标准化（公式 3-1 至 3-3）三步串行流程统一了输入数据的尺度与分布。缺失值填充模块在传统三方法之外，自研设计了对称 Encoder-Decoder Autoencoder 网络（公式 3-4）配合 Masked Reconstruction 训练策略（公式 3-5），并提出 Mask-then-Impute 评估框架（公式 3-6 至 3-8）以客观比较各方法精度；在 Benchmark 数据集上的实验中，Autoencoder 方法 RMSE 最低（0.2249），分别优于 KNN（降低 24.5%）和均值填充（降低 77.5%）。批次效应校正实现了自研 Baseline 位置尺度对齐（公式 3-9）和基于 neuroCombat 的 ComBat 安全封装两种方法，并构建了 PCA + Silhouette（公式 3-10）+ 批次质心距离（公式 3-11）的双维度评估体系；Baseline 校正后批次质心距离由 5.38 降至约 0，PC1 解释方差比由 22.0% 提升至 50.9%。下游分析涵盖 Welch t 检验（公式 3-12）+ BH-FDR + log2FC（公式 3-13）的差异代谢物挖掘、超几何 KEGG 通路富集（公式 3-14），以及 MetaKG 三库知识图谱一跳邻域溯源；以 P1_AA_0001 vs P1_AA_1024 为例，检出 538 个显著差异代谢物，富集到 D-Amino acid metabolism 通路（q = 0.000206），结果与实验设计预期高度一致。系统通用性验证表明，系统能够适应不同规模和注释覆盖度的代谢组学数据集，具有良好的通用性与鲁棒性。对应的 Web 工程实现细节将在第四章中详细描述。
...
本章对系统的总体设计与工程实现进行了系统性阐述。系统采用前后端分离架构，后端以 FastAPI 框架实现 RESTful API，内部分为 API 层、Service 层、Algorithm 层和 Repository 层；数据库采用 SQLite + SQLAlchemy ORM，以混合存储策略兼顾结构化查询和大文件高效存储；系统按分析流程划分为六个功能模块，以线性 Pipeline 结构组织数据处理流程，通过任务状态机追踪整体处理进度。在工程实现层面，后端 API 接口分组设计规范，Benchmark 数据集采用离线预计算策略，run_combat_safe() 安全封装保证鲁棒性；前端 15 个功能组件职责清晰，六类可视化图表支持丰富用户交互，DatasetSelector 支持四数据集无缝切换；功能测试和边界情况测试覆盖率良好，四数据集完整流程测试验证了系统的通用性，所有测试均通过。
