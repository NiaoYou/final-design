...
代谢组学数据具有若干区别于一般机器学习数据集的典型特点。第一是高维小样本特性，在典型的非靶向代谢组学实验中，通过 LC-MS 检测得到的特征数目可达数百至数千个，而单次实验的样本量通常仅为数十至数百个，高维数据中特征维度远超样本维度（p >> n），会显著增加统计方法的不稳定性和过拟合风险[5]。第二是特征间复杂相关结构，生物体内代谢物通过代谢通路彼此连接，参与同一通路的代谢物在丰度变化上通常呈现出高度相关性，这种内生的相关结构使得能够捕获非线性依赖关系的方法（如 Autoencoder）在缺失值填充等任务上具有潜在优势。第三是数据分布偏斜与异质性，LC-MS 原始数据通常呈右偏分布，且不同代谢物信号强度量级差异悬殊，常需进行 log 变换和尺度归一化[16]，不同批次间的系统性分布差异则构成了批次效应问题的本质。第四是普遍存在缺失值，由于仪器检测下限、谱峰拾取算法误差以及低丰度代谢物的随机检测失败，代谢组学数据矩阵的缺失比例在 20%～50% 之间较为常见。

针对上述数据特点，代谢组学数据分析须经历一套较为固定的预处理流程：格式检验与质控（核查矩阵完整性、排查异常样本与低质量特征）→ 缺失值填充 → 归一化与尺度化（log 变换、Z-score 标准化）→ 批次效应检测（PCA、Silhouette 系数、批次质心距离）→ 批次效应校正 → 校正效果评估。上述各步骤在本系统中均有对应的算法实现和 Web 交互界面支持，具体设计详见第三章。
...
本系统集成两种批次校正方法：一是 Baseline 逐特征位置尺度对齐方法，二是基于 neuroCombat 安全封装的 ComBat 经验 Bayes 方法。需要特别说明的是，Baseline 方法的算法思想本身（Per-feature Location-Scale Normalization）属于文献中长期使用的经典策略（详见第二章 2.3.1 节），并非本文提出的新方法，也未对该算法做出方法层面的改进；本文在批次校正模块中的工程贡献是：未引入任何外部统计库，基于 NumPy 广播自研编写了该算法的 Python 实现（对应 `per_feature_batch_location_scale_baseline()` 函数），并将其作为快速基线方法整合到平台中，与基于 neuroCombat 的 ComBat 经验 Bayes 方法并列提供给用户选择 —— 前者实现简单、可解释性强，作为快速基线方法；后者支持生物学协变量保护，适用于小批次精细化校正场景。

### 3.3.1 Baseline 逐特征位置尺度对齐

如前所述，Baseline 方法所采用的算法思想是经典的逐特征位置尺度归一化策略（其原理与文献定位见第二章 2.3.1 节），其核心假设为：批次效应主要表现为各批次在每个特征上的位置（均值）和尺度（标准差）的系统性偏移，通过将各批次分布对齐到全局分布消除偏差。本小节给出本文对该经典思想所做的具体公式形式与自研 NumPy 实现。对于特征 $j$，设全局均值 $\mu_j$、全局标准差 $\sigma_j$，批次 $b$ 内均值 $\mu_{bj}$、批次内标准差 $\sigma_{bj}$，则属于批次 $b$ 的样本 $i$ 的校正公式为：
...
结果分析：Baseline 方法校正后批次质心距离归零，表明 7 个批次质心在 PCA 前两个主成分空间中实现了完全重合，批次效应被有效消除。校正后 PC1 解释方差比由 22.0% 大幅提升至 50.9%，表明主要方差来源从批次差异转变为潜在的生物学结构差异。批次 Silhouette 系数向 0 方向移动（而非向 −1 方向），这一现象源于校正前后 PCA 坐标系的根本性变化，不同坐标系下的 Silhouette 系数不具有直接可比性；批次质心距离（完全归零）是更直接、更可靠的评估依据。

为验证 ComBat 的行为，本文在 Benchmark 数据集上同样运行了 ComBat 经验 Bayes 校正（不传入生物学协变量）。校正后批次质心距离为 0.0194，批次 Silhouette 系数为 −0.0437，分组 Silhouette 系数为 −0.4469，三项指标均与 Baseline 高度接近（Baseline 对应值为 ≈ 0、−0.0343、−0.4465），符合理论预期——Benchmark 各批次样本量均匀（每批 245 个样本），经验 Bayes 收缩的稳定性优势在样本量充足时无法体现。ComBat 的实际价值在于批次大小不均衡或需传入生物学协变量保护差异的场景。Benchmark 数据集校正前后 PCA 散点对比如图 3-4 所示，批次质心距离的量化变化如图 3-5 所示。
...
本系统以代谢组学研究人员为主要目标用户，其使用场景通常为：拥有来自多批次 LC-MS 实验的代谢组学数据矩阵，需要依次完成缺失值填充、批次效应校正和差异代谢物分析，并希望通过可视化界面直观评估各步骤的处理效果。围绕这一核心场景，系统功能需求可划分为六类。

数据管理方面，系统支持 CSV 和 XLSX 格式上传（文件上限 50MB），支持 Long Format 数据格式的列名自定义映射（特征列、样本列、数值列、批次标签列、分组标签列），支持 Benchmark、BioHeart、MI、AMIDE 等预置数据集与自定义上传数据集的切换，并提供数据集基本信息预览。数据预处理方面，系统支持可配置的缺失率过滤（默认阈值 50%）、log1p 变换和 Z-score 标准化，预处理结果以 CSV 矩阵持久化存储。缺失值填充方面，系统集成均值、中位数、KNN（$K$ 可配置）、Autoencoder 四种方法，支持 Mask-then-Impute 评估结果展示，填充矩阵可下载。批次效应校正方面，系统支持 Baseline 和 ComBat 两种方法，提供校正前后 PCA 散点图对比及三项定量评估指标（批次质心距离、批次 Silhouette、分组 Silhouette），校正矩阵可下载。下游分析方面，系统实现了 Welch t 检验 + BH-FDR 差异代谢物分析（火山图）、KEGG 超几何富集分析（气泡图）和 MetaKG 力导向知识图谱溯源（支持拖拽、搜索、类型过滤）。结果输出方面，核心中间矩阵支持 CSV 下载，评估报告以 JSON 持久化存储，前端按需读取。

在非功能需求方面，系统的设计重点关注以下几点。可用性上，界面面向无编程背景的生物医学研究人员，关键参数均提供默认值，错误信息以友好文字形式提示，不终止整体流程。可扩展性上，算法层以独立模块组织，新增填充方法或校正方法只需添加实现文件，无需修改上层 Service 与 API 代码，前端组件化设计同理。可靠性上，ComBat 等可能失败的算法均有安全封装与降级机制，捕获异常后退回备用方法而不崩溃。性能上，以本地单机部署为目标，针对 Benchmark 数据集规模的处理耗时在可接受范围内（预处理 < 30s，KNN 填充 < 60s，Autoencoder 填充 < 90s）。
