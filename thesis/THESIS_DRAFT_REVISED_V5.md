...
为验证 ComBat 的行为，本文在 Benchmark 数据集上同样运行了 ComBat 经验 Bayes 校正（不传入生物学协变量）。校正后批次质心距离为 0.0194，批次 Silhouette 系数为 −0.0437，与 Baseline 结果高度接近，符合理论预期——Benchmark 各批次样本量均匀（每批 245 个样本），经验 Bayes 收缩的稳定性优势在样本量充足时无法体现。ComBat 的实际价值在于批次大小不均衡或需传入生物学协变量保护差异的场景。Benchmark 数据集校正前后 PCA 散点对比如图 3-4 所示，批次质心距离的量化变化如图 3-5 所示。
