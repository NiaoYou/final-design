...
由 [`scipy.stats.ttest_ind`](scipy.stats:ttest_ind)（`equal_var=False`）计算 $p$ 值。对全部特征的 $p$ 值采用 Benjamini-Hochberg FDR 校正得到 $q$ 值（[`statsmodels.stats.multitest.multipletests`](statsmodels.stats.multitest:multipletests)，备用手动实现）。
...
首先，Autoencoder 填充尚未完全集成到 Web 在线任务流。当前 Autoencoder 填充以离线预计算方式运行，用户无法通过 Web 界面为自定义上传数据集在线触发 Autoencoder 填充，主要原因是模型训练耗时较长，可能超出 HTTP 同步请求的超时限制，故暂时牺牲了交互层面的易用性。
