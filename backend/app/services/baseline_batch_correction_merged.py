    ...
    *,
    ...
    method_report: Dict[str, Any] = {
        ...
        "strict_combat": {
            "status": "implemented",
            "note": (
                "严格 ComBat（pyComBat 经验 Bayes 实现）已在本项目中实现，"
                "可通过 run_batch_correction_matrix(method='combat') 调用，"
                "或在 run_merged_benchmark_pipeline.py 中设置 --batch-method combat 启用。"
                "当前主链路（baseline_batch_correction_merged）仍展示 baseline 结果，"
                "ComBat 结果在 evaluation/方法对比区块中展示。"
            ),
            "library": "neuroCombat（pip install neuroCombat）",
